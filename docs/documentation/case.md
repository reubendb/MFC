...
| `stl%filepath`      | String  | Path to the STL file.                                        |
| `stl%scale(i)`      | Real    | STL model's (applied) scaling factor for component $i$.      |
| `stl%rotate(i)`     | Real    | STL model's (applied) angle of rotation about axis $i$.      |
...
- 'stl%scale', 'stl%rotate` and `stl%offset` define how the model should be transformed to domain-space by first
scaling by `stl%scale`, then rotating about the Z, X, and Y axes (using `stl%rotate`), and finally translating by `stl%offset`.
...
| 20   | STL                | 3     | N      | Imports an STL model. Requires `stl%filepath`. | |
