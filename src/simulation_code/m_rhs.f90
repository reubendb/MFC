...
MODULE m_rhs
    ...
    CONTAINS
        ...
        !!     @param q_cons_vf Cell-average conservative variables
        !!     @param p_star equilibrium pressure at the interface    
        SUBROUTINE s_compute_p_prelax(pstar,alpha1,p_k,alpha_k)

            REAL(KIND(0d0)), INTENT(OUT)                          :: pstar, alpha1
                        ...
                        IF (mpp_lim) THEN
                            ...
                            DO i = 1, num_fluids
                                IF ((q_cons_vf(i+cont_idx%beg-1)%sf(j,k,l) .LT. 0d0) .OR. &
                                    ...
                                    q_cons_vf(i+cont_idx%beg-1)%sf(j,k,l) = sgm_eps
                                    q_cons_vf(i+adv_idx%beg-1)%sf(j,k,l)  = sgm_eps
                                    q_cons_vf(i+internalEnergies_idx%beg-1)%sf(j,k,l)  = sgm_eps
                                END IF
                                IF (q_cons_vf(i+adv_idx%beg-1)%sf(j,k,l) .GT. 1d0) & 
                                    q_cons_vf(i+adv_idx%beg-1)%sf(j,k,l) = 1d0-sgm_eps
        ...
        END SUBROUTINE s_compute_pt_prelax
        ...
        SUBROUTINE s_infinite_pt_relaxation(q_cons_vf) ! ----------------
            ...
            REAL(KIND(0d0)), DIMENSION(num_fluids)            ::          Tk
            REAL(KIND(0d0))                                   ::   sum_alpha
            REAL(KIND(0d0))                                   ::   rho, rhoe
            REAL(KIND(0d0))                                   ::rhoalpha1, rhoalpha2
            REAL(KIND(0d0))                                   ::          a1
            REAL(KIND(0d0))                                   ::    dyn_pres
            REAL(KIND(0d0))                                   ::        E_We
            REAL(KIND(0d0))                                   ::gamma, pi_inf
            REAL(KIND(0d0))                                   ::      p_infk
        ...
        !!     @param q_cons_vf Cell-average conservative variables
        !!     @param p_star equilibrium pressure at the interface    
        SUBROUTINE s_compute_ptg_pTrelax_old(pstar,Tstar,rho0,E0,failed)
            ...
            DO WHILE (DABS(delta/pstar) .GT. ptgnewton_eps) 
                  ...
                  Tstar = (-bp + sqrt(bp*bp - 4.d0*ap*dp))/(2.d0*ap)
                  ! Convergence
                  IF ((iter .GT. ptgnewton_iter) .OR. ISNAN(pstar) .OR. ISNAN(Tstar) .OR. & 
                  ...
                  pstar = pstar - delta
            END DO
        END SUBROUTINE s_compute_ptg_pTrelax_old !-------------------------------
